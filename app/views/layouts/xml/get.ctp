...
<?php
	...
	if(($hotspot['Hotspot']['description'])=='default')
		    {
		        $hotspot['description'] == 'Klickable';
		    }		
	if($hotspot['Hotspot']['url']=='default')
	    {
...
<hotspot id="<?php echo $hotspot['Hotspot']['id']; ?>" 
		...
		thumb="<?php echo $hotspot['HotspotThumb']['filename']; ?>"
		description="<?php echo Sanitize::clean($hotspot['Hotspot']['description'], array('unicode'=>true)); ?>" 
		url="<?php echo htmlentities($hotspot['Hotspot']['url']); ?>"  
	/>
<?php endforeach; ?>
