...
class SwfUploadComponent extends Object {
	...
	function write() {
		...
		if (!$folder) {
			...
			if (!($moved = move_uploaded_file($this->filedata['tmp_name'], $this->uploadpath . $this->filename))) {
	 ...
	 */
	function validate() {
		$post_ok = isset($this->filedata);
		$upload_error = $this->filedata['error'];
		$got_data = (is_uploaded_file($this->filedata['tmp_name']));
