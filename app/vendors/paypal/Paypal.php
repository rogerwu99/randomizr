...
//		token?
	    ...
	    $profile_id = urlencode($paymentInfo['PROFILEID']);
	   $amt = urlencode($paymentInfo['AMT']);
		 $currencyCode="USD";
	   ...
	   $description = urlencode($paymentInfo['Description']);
	$nvpstr = "&IPADDRESS=$ip&CURRENCYCODE=$currencyCode&PROFILESTARTDATE=$startDate";

			$nvpstr .= '&PROFILEID='.$profile_id.'&AMT='.$amt.'&DESC='.$description;
	...
	function ManageRecurringPaymentsProfileStatus($paymentInfo=array()){
			...
			$action = urlencode("Cancel");
	   		$nvpstr = '&PROFILEID='.$profile_id.'&ACTION='.$action;
