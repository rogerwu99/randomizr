...
class OAuth_Consumer { 
	...
	protected function createOAuthToken($response) {
		if (isset($response['oauth_token']) && isset($response['oauth_token_secret'])) {
			if (isset($response['application_name'])){
				return new OAuthToken($response['oauth_token'], $response['oauth_token_secret'], $response['application_name']);
			}
			elseif (isset($response['user_id'])){
				return new OAuthToken($response['oauth_token'], $response['oauth_token_secret'], NULL ,$response['user_id']);
			}
			else {
